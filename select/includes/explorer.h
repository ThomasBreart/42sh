...
void					print_time(time_t *time, int mask, int fd);
void					del(t_file **f);
t_file					*ft_open(t_args *a, char *dirname);
int						sort_files(t_args *a, t_file *root, t_file *f);
void					parcours(int fd, struct winsize *ws, t_args *a,
							t_file *f);
...
int						skip_pages(struct winsize *ws, int *cur,
							t_file **ptr, t_args *a);
t_file					*do_input(t_term *t, t_file *f, t_args *arg,
							char **path);
int						reset_pages(int y, t_term *t, t_args *arg);
